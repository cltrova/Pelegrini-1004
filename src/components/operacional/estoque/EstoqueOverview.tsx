...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, CircleDollarSign, Filter, Package, PackageCheck, RefreshCw, ShoppingCart, TrendingUp } from 'lucide-react';
...
function Metric({ label, value, icon: Icon, tone = 'normal', onClick }: { label: string; value: string; icon: typeof Package; tone?: string; onClick?: () => void }) {
  const content = <><Icon className="h-3.5 w-3.5 text-muted-foreground" /><span className="truncate text-[10px] uppercase tracking-wide text-muted-foreground">{label}</span><strong className={cn('ml-auto truncate text-sm tabular-nums', tone === 'danger' && 'text-red-400', tone === 'attention' && 'text-amber-300', tone === 'positive' && 'text-emerald-400')}>{value}</strong></>;
  return onClick ? <button className="flex min-w-0 items-center gap-1.5 border-r border-border/60 px-2.5 text-left last:border-r-0 hover:bg-muted/30" onClick={onClick} type="button">{content}</button> : <div className="flex min-w-0 items-center gap-1.5 border-r border-border/60 px-2.5 last:border-r-0">{content}</div>;
}

function Panel({ title, action, children, className }: { title: string; action?: ReactNode; children: ReactNode; className?: string }) {
  return <section className={cn('min-h-0 min-w-0 border border-border/70 bg-card/60 p-2.5', className)}><div className="mb-2 flex items-center justify-between gap-2"><h2 className="text-xs font-semibold text-foreground">{title}</h2>{action}</div>{children}</section>;
}
...
export function EstoqueOverview({ stockData, movementData, activeCompanyCode, onOpenCentral, isFetching = false, onRefresh }: Props) {
  ...
  const topValue = useMemo(() => [...filtered].sort((a, b) => b.valor_estoque - a.valor_estoque).slice(0, 5), [filtered]);
  const topRisk = useMemo(() => [...filtered].filter((product) => product.status === 'faltando' || product.status === 'alerta').sort((a, b) => (a.cobertura_meses ?? -1) - (b.cobertura_meses ?? -1)).slice(0, 5), [filtered]);
  const topExcess = useMemo(() => [...filtered].filter((product) => product.status === 'excesso').sort((a, b) => b.valor_estoque - a.valor_estoque).slice(0, 5), [filtered]);
  ...
  return <div aria-label="Visão geral do estoque" className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
    <div aria-label="Filtros da visão geral" className="flex min-h-10 shrink-0 flex-wrap items-center gap-1.5 border-b border-border/60 bg-background/95 px-2.5 py-1.5" role="toolbar">
      <Filter className="h-3.5 w-3.5 text-muted-foreground" />
      <select aria-label="Período da visão geral" className="h-7 border border-border bg-card px-2 text-xs" value={months} onChange={(event) => setMonths(Number(event.target.value))}><option value={3}>Últimos 3 meses</option><option value={6}>Últimos 6 meses</option><option value={12}>Últimos 12 meses</option></select>
      <select aria-label="Filtrar por marca" className="h-7 max-w-40 border border-border bg-card px-2 text-xs" value={brand} onChange={(event) => setBrand(event.target.value)}><option value="">Todas as marcas</option>{options.brands.map((item) => <option key={item}>{item}</option>)}</select>
      <select aria-label="Filtrar por grupo" className="h-7 max-w-40 border border-border bg-card px-2 text-xs" value={group} onChange={(event) => setGroup(event.target.value)}><option value="">Todos os grupos</option>{options.groups.map((item) => <option key={item}>{item}</option>)}</select>
      <select aria-label="Filtrar por filial" className="h-7 max-w-40 border border-border bg-card px-2 text-xs" value={branch} onChange={(event) => setBranch(event.target.value)}><option value="">Todas as filiais</option>{options.branches.map((item) => <option key={item}>{item}</option>)}</select>
      <span className="ml-auto text-[11px] text-muted-foreground">{number.format(filtered.length)} produtos analisados</span>
      {onRefresh && <Button aria-label="Atualizar visão geral" className="h-7 w-7" disabled={isFetching} onClick={onRefresh} size="icon" type="button" variant="ghost"><RefreshCw className={cn('h-3.5 w-3.5', isFetching && 'animate-spin')} /></Button>}
    </div>
    ...
    <div className="grid min-h-0 flex-1 gap-2 overflow-y-auto p-2 lg:grid-cols-12">
      ...
      <Panel className="lg:col-span-7" title="Comparativo estoque, vendas e cobertura"><div className="h-36"><ResponsiveContainer height="100%" width="100%"><BarChart data={summary.status}><CartesianGrid stroke="hsl(var(--border))" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" tick={{ fontSize: 10 }} /><YAxis allowDecimals={false} tick={{ fontSize: 10 }} width={30} /><Tooltip /><Bar dataKey="value" fill="#38bdf8" name="Produtos" radius={[3, 3, 0, 0]} onClick={(item) => { if (item?.name === 'Ruptura') onOpenCentral('out'); }} /></BarChart></ResponsiveContainer></div></Panel>
      <Panel className="lg:col-span-5" title="Leituras rápidas"><div className="grid grid-cols-2 gap-2 text-xs"><div className="border border-border/60 p-2"><span className="text-muted-foreground">Cobertura média</span><strong className="mt-1 block">{summary.averageCoverage === null ? 'Sem dados' : `${summary.averageCoverage.toFixed(1)} meses`}</strong></div><div className="border border-border/60 p-2"><span className="text-muted-foreground">Sem venda</span><strong className="mt-1 block">{number.format(summary.noSale)} produtos</strong></div><div className="border border-border/60 p-2"><span className="text-muted-foreground">Valor em excesso</span><strong className="mt-1 block text-amber-300">{currency.format(summary.excessValue)}</strong></div><div className="border border-border/60 p-2"><span className="text-muted-foreground">Movimentações</span><strong className="mt-1 block">{number.format(summary.movements)}</strong></div></div></Panel>
    </div>
  </div>;
}
...
function Ranking({ items, value, onSelect, danger, attention }: { items: Array<{ produto: string; marca: string; cod_produto: number }>; value: (item: any) => string; onSelect: () => void; danger?: boolean; attention?: boolean }) {
  return items.length ? <div className="space-y-1">{items.map((item, index) => <button className="flex w-full items-center gap-2 border-b border-border/40 px-1 py-1 text-left last:border-0 hover:bg-muted/30" key={`${item.cod_produto}-${index}`} onClick={onSelect} type="button"><span className="w-4 text-[10px] text-muted-foreground">{index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-medium">{item.produto}</span><span className="block truncate text-[10px] text-muted-foreground">{item.marca}</span></span><span className={cn('text-xs tabular-nums', danger && 'text-red-400', attention && 'text-amber-300')}>{value(item)}</span></button>)}</div> : <div className="flex min-h-20 items-center justify-center text-xs text-muted-foreground">Sem dados no filtro.</div>;
}
